...
from app.api.deps import get_current_user_id, get_owned_notebook
...
async def create_notebook(
    ...
    notebook = Notebook(
        owner_id=user_id,
        name=payload.name.strip() or "Untitled notebook",
        description=payload.description,
    )
    db.add(notebook)
    await db.commit()
    await db.refresh(notebook)
    return _to_out(notebook, 0)
...
async def delete_notebook(
    ...
    upload_dir = os.path.join(settings.UPLOAD_DIR, str(notebook.id))
